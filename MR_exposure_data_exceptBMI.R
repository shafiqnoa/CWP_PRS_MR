...
AFB_ctg_vl <- afb[,c(2,3,1,4,5,6,10,11,8)]
colnames(AFB_ctg_vl) <- c("CHR","BP","SNP","A1","A2","FREQ","BETA","SE","P")
fwrite(AFB_ctg_vl, file = "AFB_ctg_vl.txt", quote = FALSE, sep = "\t")
...
cwp_clump <- fread("cwp_clumped_snp_1e05.txt") #39 snps
...
mdd_exposure <- mdd_ins[which(mdd_clump$SNP%in%mdd_ins$SNP),]
fwrite(mdd_exposure, file = "mdd_exposure.txt", sep = "\t", quote = FALSE)

cwp_exposure <- cwp_ins_e05[which(cwp_clump$SNP%in%cwp_ins_e05$SNP),]
fwrite(cwp_exposure, file = "cwp_exposure.txt", sep = "\t", quote = FALSE)

afb_exposure <- AFB_1e05[which(afb_clump$SNP%in%AFB_1e05$SNP),]
fwrite(afb_exposure, file = "afb_exposure.txt", sep = "\t", quote = FALSE)

school_exposure <- yr_school_1e05[which(school_clump$SNP%in%yr_school_1e05$SNP),]
